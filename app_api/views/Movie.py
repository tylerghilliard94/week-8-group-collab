...
class MovieView(ViewSet):
    ...
    def list(self, request):
        ...
        for movie in movies:
            movie.watched = request.auth.user.id
    ...
    def movies_under100(self, request):
        movies = Movie.objects.filter(run_time__lte=120)
        serializer = MovieSerializer(movies, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    @action(methods=["get"], detail=False)
    def movies_genre_start_with_h(self, request):
        movies = Movie.objects.filter(genre__name__endswith="s")
        serializer = MovieSerializer(movies, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    @action(methods=["get"], detail=False)
    def movies_genre_horror_or_christmas(self, request):
...
class MovieSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ("id", "title", "description", "run_time",
                  "user", "date_released", "genre", "watched")
        depth = 1
